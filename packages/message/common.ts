...
const eventTargetClone = (process.env.VI_TESTING === "true" ? new EventTarget() : window) as any;

// 避免页面载入后改动 EventTarget.prototype 的方法导致消息传递失败
export const pageDispatchEvent = eventTargetClone.dispatchEvent.bind(eventTargetClone);
export const pageAddEventListener = eventTargetClone.addEventListener.bind(eventTargetClone);
export const pageRemoveEventListener = eventTargetClone.removeEventListener.bind(eventTargetClone);
...
export const pageDispatchCustomEvent = <T = any>(eventType: string, detail: T) => {
  if (detailClone && detail) detail = <T>detailClone(detail, eventTargetClone);
