...
// Overwrite `./dist/ext/manifest.json` with the cleaned chromeManifest to prevent warnings when loaded as an unpacked extension or packed as a CRX
await fs.writeFile("./dist/ext/manifest.json", JSON.stringify(chromeManifest, null, 2));
...
const writeZip = (zip, name) => {
  return new Promise((resolve, reject) => {
    zip
      ...
      .on("finish", () => {
        console.log(`Zip file ${name} created.`);
        resolve();
      })
      .on("error", reject);
  });
...
await writeZip(chrome, `./dist/${packageInfo.name}-v${packageInfo.version}-chrome.zip`);

if (PACK_FIREFOX) {
  await writeZip(firefox, `./dist/${packageInfo.name}-v${packageInfo.version}-firefox.zip`);
}

// 处理crx
try {
  const privateKey = await fs.readFile("./dist/scriptcat.pem", { encoding: "utf8" });
  ...
  console.log("No private key (scriptcat.pem) found in dist, skipping CRX packaging.");
}
