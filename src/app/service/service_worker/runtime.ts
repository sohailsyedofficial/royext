...
import { checkVIPStatus } from "@App/app/vip-service";
...
export class RuntimeService {
  ...
  init() {
    ...
    chrome.storage.onChanged.addListener(async (changes, areaName) => {
      if (areaName === "local" && changes.vip_active) {
        if (changes.vip_active.newValue) {
          this.logger.info("VIP status activated. Registering user scripts.");
          await this.registerUserscripts();
        } else {
          this.logger.info("VIP status deactivated. Unregistering user scripts.");
          await this.unregisterUserscripts();
        }
        this.updateIcon();
      }
