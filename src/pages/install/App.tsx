...
const fetchScriptBody = async (url: string, { onProgress }: { [key: string]: any }) => {
  ...
  let code = await readRawContent(chunksAll, contentType);


  // VIP scripts are served as plaintext (secured via license validation on the server)
...
function App() {
  ...
  useEffect(() => {
    ...
    if (upsertScript) {
      document.title = `${!isUpdate ? t("install_script") : t("update_script")} - ${i18nName(upsertScript!)} - RoyExt`;
    }
  ...
  const urlHref = useMemo(() => {
    if (searchParamUrl) {
      try {
        ...
        const rawUrl = idx !== -1 ? decodeURIComponent(location.search.slice(idx + 4)) : searchParamUrl;
