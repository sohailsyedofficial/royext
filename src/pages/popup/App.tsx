  ...
  useEffect(() => {
    const loadAnnouncementsData = async () => {
      try {
        ...
        console.warn("Failed to load announcements in popup:", e);
      }
    };
  ...
  const handleMenuClick = async (key: string) => {
    switch (key) {
      ...
      case "checkUpdate":
        ...
        break;
      case "report_issue": {
        window.open("/src/options.html#/shop?action=feedback", "_blank");
        break;
      }
      default:
              ...
              <Dropdown
                ...
                droplist={
                  ...
                  >
                    ...
                    <Menu.Item
                      key="https://github.com/sohailsyedofficial/royext"
        ...
        <div className="tw-flex tw-flex-row arco-card-header !tw-h-6">
          {versionCompare(ExtVersion, checkUpdate.version) === VersionCompare.LESS ? (
            <Tooltip content={`${t("popup.new_version_available")} (v${checkUpdate.version})`}>
              <span
                onClick={() => {
                  window.open(`https://github.com/sohailsyedofficial/royext/releases/tag/v${checkUpdate.version}`);
                }}
                className={`tw-text-[12px] tw-font-500 tw-cursor-pointer tw-underline tw-underline-offset-2 tw-text-purple-500 dark:tw-text-purple-400`}
              >{`v${ExtVersion}`}</span>
            </Tooltip>
          ) : checkUpdateStatus === 0 ? (
