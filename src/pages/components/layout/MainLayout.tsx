...
import VIPLock from "@App/pages/components/VIPLock";
import { fetchAnnouncements, type VIPAnnouncement } from "@App/app/vip-service";
import VipBadgeIcon, { 
  VipShopIcon, 
  DeveloperIcon, 
  ScriptManagerIcon, 
  ...
  ToolsIcon, 
  SettingsIcon 
} from "@App/pages/components/VipBadgeIcon";
    ...
    if (pageName === "options") {
      const loadAnnouncements = async () => {
        try {
          ...
          console.warn("Failed to load announcements in MainLayout:", e);
        }
      };
      loadAnnouncements();
    }
  ...
  return (
    <ScrollBoundary parentNodeSelector="#root">
      ...
      >
        <VIPLock>
          {contextHolder}
          <Layout className={"tw-min-h-screen tw-relative tw-overflow-hidden"}>
            {/* Cosmic background neon blobs for high-end aesthetic */}
            <div className="futuristic-glow-orb orb-1" />
            <div className="futuristic-glow-orb orb-2" />
            <div className="futuristic-glow-orb orb-3" />
            ...
            >
              ...
              <div className="tw-flex tw-flex-row tw-items-center">
                <img style={{ height: "36px", marginRight: "8px" }} src="/assets/logo.png" alt="RoyExt" className="royext-logo-img" />
                <span className="royext-logo-text">RoyExt</span>
              </div>
              
              {pageName === "options" && (
                <div className="header-nav-menu">
                  <NavLink to="/" className={({ isActive }) => `nav-tab-item ${isActive ? 'active' : ''}`}>
                    <ScriptManagerIcon size={14} /> <span>{t("installed_scripts")}</span>
                  </NavLink>
                  <NavLink to="/subscribe" className={({ isActive }) => `nav-tab-item ${isActive ? 'active' : ''}`}>
                    <SubscribeIcon size={14} /> <span>{t("subscribe")}</span>
                  </NavLink>
                  <NavLink to="/logger" className={({ isActive }) => `nav-tab-item ${isActive ? 'active' : ''}`}>
                    <LogsIcon size={14} /> <span>{t("logs")}</span>
                  </NavLink>
                  <NavLink to="/tools" className={({ isActive }) => `nav-tab-item ${isActive ? 'active' : ''}`}>
                    <ToolsIcon size={14} /> <span>{t("tools")}</span>
                  </NavLink>
                  <NavLink to="/setting" className={({ isActive }) => `nav-tab-item ${isActive ? 'active' : ''}`}>
                    <SettingsIcon size={14} /> <span>{t("settings")}</span>
                  </NavLink>
                  <NavLink to="/vip" className={({ isActive }) => `nav-tab-item ${isActive ? 'active' : ''}`}>
                    <VipBadgeIcon size={14} /> <span>VIP</span>
                  </NavLink>
                  <NavLink to="/shop" className={({ isActive }) => `nav-tab-item ${isActive ? 'active' : ''}`}>
                    <VipShopIcon size={14} /> <span>Vault</span>
                  </NavLink>
                  <NavLink to="/developer" className={({ isActive }) => `nav-tab-item ${isActive ? 'active' : ''}`}>
                    <DeveloperIcon size={14} /> <span>Developer</span>
                  </NavLink>
                </div>
              )}
                ...
                {pageName === "options" && (
                  ...
                  >
                    <Button
                      type="primary"
                      size="small"
                      className="futuristic-action-btn"
                    >
                    ...
                    <Menu>
                      {languageList.map((value) => (
                        <Menu.Item
                          ...
                          onClick={() => {
                            if (value.key === "help") {
                              window.open("https://github.com/sohailsyedofficial/royext/discussions", "_blank");
                              return;
                            }
          ...
          {pageName === "options" && announcements.length > 0 && (
                  ...
                  action={
                    ann.btn_text && ann.btn_link ? (
                      <Button
                        ...
                        onClick={() => window.open(ann.btn_link, "_blank")}
                      >
                        {ann.btn_text}
                      </Button>
                    ) : undefined
